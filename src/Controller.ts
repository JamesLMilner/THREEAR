...
export class Controller extends THREE.EventDispatcher {
	...
	private _initArtoolkit(onCompleted: () => any) {
		...
		const cameraParameters = new ARToolKitCameraParam(
			...
			() => {
				...
				(this.arController.ctx as any).mozImageSmoothingEnabled = this.parameters.imageSmoothingEnabled;
				(this.arController.ctx as any).webkitImageSmoothingEnabled = this.parameters.imageSmoothingEnabled;
				(this.arController.ctx as any).msImageSmoothingEnabled = this.parameters.imageSmoothingEnabled;
				(this.arController.ctx as any).imageSmoothingEnabled = this.parameters.imageSmoothingEnabled;
	...
	private onMarkerFound(event: any, minConfidence: number, markerObject: Object3D) {
	 ...
	 */
	private updateWithModelViewMatrix(modelViewMatrix: THREE.Matrix4, markerObject: Object3D): boolean {
		...
		if (this.parameters.changeMatrixMode === "modelViewMatrix") {
			if (this.parameters.positioning.smooth) {
				...
				if (this.smoothMatrices.length < (this.parameters.positioning.smoothCount + 1)) {
					...
					if (exceedsAverageTolerance >= this.parameters.positioning.smoothThreshold) {
